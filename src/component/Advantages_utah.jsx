...
export const Utahadvantages = () => {
    const [isLoading, setLoading] = useState(false);
    const [first_name_1, setFirst_name] = useState("");
    const [last_name_1, setLast_name] = useState("");
    const [state_1, setState] = useState("");
    const [email_1, setEmail] = useState("");
    const [phone_number_1, setPhone_number] = useState("");
    const [information_1, setInformation] = useState("");
    ...
    let location_name_1 = "utah"
    const [result, setResult] = useState([]);
    function Advantageformdata(e) {
      setLoading(true);
      e.preventDefault();
      let data = { location_name_1, first_name_1, last_name_1, state_1, email_1, phone_number_1, information_1 }
      fetch(`${siteURL}Save_enquiry_utah`, {
        method: "POST",
        headers: {
                ...
                <div className="body">
                  <div className="container">
                    <div className="main-stuff">
                      ...
                      <MemberAdvantageRating.UtahReview/>
                    </div>
                    <div className="sidebar">
                          ...
                          <p>Fill out this short form and an Acclaimed Home Warranty representative will follow up on your request.</p>
                          {
                            showtext == true ? <Thankyou /> :
                              <div className="everest-forms">
                                ...
                                <div className="form">
                                  <form>
                                    <label><input type="text" placeholder="FirstName*" name="FirstName" value={first_name_1} onChange={(e) => { setFirst_name(e.target.value) }} /></label>
                                    <label><input type="text" placeholder="LastName*" name="LastName" value={last_name_1} onChange={(e) => { setLast_name(e.target.value) }} /></label>
                                    <label>
                                      <select value={state_1} onChange={(e) => { setState(e.target.value) }}>
                                        {statelist.map((state) => (
                                          <option value={state.state_name} key={state.state_name}>{state.state_name}</option>
                                        ))}
                                      </select>
                                    </label>
                                    <label><input type="email" placeholder="Email*" name="email" value={email_1} onChange={(e) => { setEmail(e.target.value) }} /></label>
                                    <label><input type="text" placeholder="PhoneNumber*" name="phonenumber" value={phone_number_1} onChange={(e) => { setPhone_number(e.target.value) }} /></label>
                                    <label><textarea placeholder="Information Requested" className="field-msg" value={information_1} onChange={(e) => { setInformation(e.target.value) }}></textarea></label>
                      ...
                      <div className="compare-box" style={{ backgroundImage: `url(${comparebox})` }}>
                        <div className="text">
                          ...
                          <a href="/order-now" className="btn">Compare Utah Plans</a>
                        </div>
                      </div>
                    </div>
                  </div>
