...
export const Arizonaadvantages = () => {
  const [isLoading, setLoading] = useState(false);
  const [first_name_5, setFirst_name] = useState("");
  const [last_name_5, setLast_name] = useState("");
  const [state_5, setState] = useState("");
  const [email_5, setEmail] = useState("");
  const [phone_number_5, setPhone_number] = useState("");
  const [information_5, setInformation] = useState("");
  ...
  let location_name_5 = "arizona"
  const [result, setResult] = useState([]);
  
  function Advantageformdata(e) {
    setLoading(true);
    e.preventDefault();
    let data = { location_name_5, first_name_5, last_name_5, state_5, email_5, phone_number_5, information_5 }
    fetch(`${siteURL}Save_enquiry_arizona`, {
      method: "POST",
      headers: {
                ...
                <div className="body">
                  <div className="container">
                    <div className="main-stuff">
                      ...
                      <MemberAdvantageRating.ArizonaReview/>
                    </div>
                    <div className="sidebar">
                          ...
                          <p>Fill out this short form and an Acclaimed Home Warranty representative will follow up on your request.</p>
                          {
                            showtext == true ? <Thankyou /> :
                              <div className="everest-forms">
                                ...
                                <div className="form">
                                  <form>
                                    <label><input type="text" placeholder="FirstName*" name="FirstName" value={first_name_5} onChange={(e) => { setFirst_name(e.target.value) }} /></label>
                                    <label><input type="text" placeholder="LastName*" name="LastName" value={last_name_5} onChange={(e) => { setLast_name(e.target.value) }} /></label>
                                    <label>
                                      <select value={state_5} onChange={(e) => { setState(e.target.value) }}>
                                        {statelist.map((state) => (
                                          <option value={state.state_name} key={state.state_name}>{state.state_name}</option>
                                        ))}
                                      </select>
                                    </label>
                                    <label><input type="email" placeholder="Email*" name="email" value={email_5} onChange={(e) => { setEmail(e.target.value) }} /></label>
                                    <label><input type="text" placeholder="PhoneNumber*" name="phonenumber" value={phone_number_5} onChange={(e) => { setPhone_number(e.target.value) }} /></label>
                                    <label><textarea placeholder="Information Requested" className="field-msg" value={information_5} onChange={(e) => { setInformation(e.target.value) }}></textarea></label>
                      ...
                      <div className="compare-box" style={{ backgroundImage: `url(${comparebox})` }}>
                        <div className="text">
                          ...
                          <a href="/order-now" className="btn">Compare Arizona Plans</a>
                        </div>
                      </div>
                    </div>
                  </div>
