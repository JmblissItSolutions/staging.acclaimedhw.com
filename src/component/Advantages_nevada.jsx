...
export const Nevadaavantages = () => {
  const [isLoading, setLoading] = useState(false);
  const [first_name_3, setFirst_name] = useState("");
  const [last_name_3, setLast_name] = useState("");
  const [state_3, setState] = useState("");
  const [email_3, setEmail] = useState("");
  const [phone_number_3, setPhone_number] = useState("");
  const [information_3, setInformation] = useState("");
  ...
  let location_name_3 = "nevada"
  const [result, setResult] = useState([]);
  function Advantageformdata(e) {
    setLoading(true);
    e.preventDefault();
    let data = { location_name_3, first_name_3, last_name_3, state_3, email_3, phone_number_3, information_3 }
    fetch(`${siteURL}Save_enquiry_nevada`, {
      method: "POST",
      headers: {
            ...
            <div className="container">
              <div className="main-stuff">
                <div className="advantages">
                  <h2>ADVANTAGES AVAILABLE <br></br>FOR A $70 SERVICE FEE</h2>
                ...
                <MemberAdvantageRating.NevadaReview />
              </div>
              <div className="sidebar">
                    ...
                    <p>Fill out this short form and an Acclaimed Home Warranty representative will follow up on your request.</p>
                    {
                      showtext == true ? <Thankyou /> :
                        <div className="everest-forms">
                          ...
                          <div className="form">
                            <form>
                              <label><input type="text" placeholder="FirstName*" name="FirstName" value={first_name_3} onChange={(e) => { setFirst_name(e.target.value) }} /></label>
                              <label><input type="text" placeholder="LastName*" name="LastName" value={last_name_3} onChange={(e) => { setLast_name(e.target.value) }} /></label>
                              <label>
                                <select value={state_3} onChange={(e) => { setState(e.target.value) }}>
                                  {statelist.map((state) => (
                                    <option value={state.state_name} key={state.state_name}>{state.state_name}</option>
                                  ))}
                                </select>
                              </label>
                              <label><input type="email" placeholder="Email*" name="email" value={email_3} onChange={(e) => { setEmail(e.target.value) }} /></label>
                              <label><input type="text" placeholder="PhoneNumber*" name="phonenumber" value={phone_number_3} onChange={(e) => { setPhone_number(e.target.value) }} /></label>
                              <label><textarea placeholder="Information Requested" className="field-msg" value={information_3} onChange={(e) => { setInformation(e.target.value) }}></textarea></label>
                ...
                <div className="compare-box" style={{ backgroundImage: `url(${comparebox})` }}>
                  <div className="text">
                    ...
                    <a href="/order-now" className="btn">Compare Nevada Plans</a>
                  </div>
                </div>
              </div>
            </div>
