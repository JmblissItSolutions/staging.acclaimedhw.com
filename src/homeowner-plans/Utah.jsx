        ...
        <div className="upgrade_list">
          {coverage.map((dataIn, i) =>
            <div key={i} className="upgrade">
              ...
              {dataIn.content ?
                <a className="content_url" id={dataIn.id} onClick={e => clickHandler(e)}>{dataIn.name} </a> : <span className="name">{dataIn.name}</span>
              }
              {dataIn.content ?
                <div>
                  <Modal size="lg" show={show == dataIn.id ? true : null} onHide={handleClose.bind(dataIn.id)} centered>
                    <Modal.Header className="modalhearder_sec" closeButton>
                      <Modal.Title>{dataIn.content.title}</Modal.Title>
                    </Modal.Header>
                    <p className="subttl">{dataIn.content.subtitle}</p>
                    <Modal.Body><div dangerouslySetInnerHTML={{ __html: dataIn.content.full_content }} /></Modal.Body>
                    <Modal.Footer>
